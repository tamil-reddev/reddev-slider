...
function ReddevSlider(props) {
    ...
    const handleChangeCommited = () => {
        console.log(`final committed value : ${sliderValues}`);
    }
    ...
    useEffect(()=>{
      if(isInitialMount.current){
        ...
        console.log(`Before moving silder value ${sliderValues}`);
        let totalValue = 100;
        ...
        console.log(`Difference btw last and current value of moved sliderIndex ${sliderIndex}, ${existingSelectedSliderValue} - ${currentSliderValue} : ${difference}`)
        //Below slider
        ...
        console.log(`Following sliders total to find the % to +/- silderIndex [${(sliderIndex)} to ${existingAllSliderArray.length-1}] = ${total}`)
        ...
        console.log(`Total after affecting below sliders ${newAllSliderArray} = ${totalOfNew}`);
        let balanceAvailableNew = totalValue - totalOfNew;
        console.log(`is Still balanceAvailable?: ${balanceAvailableNew}`)
        ...
        if(balanceAvailableNew > 0 && sliderIndex !== (newAllSliderArray.length -1)){
          ...
          if(balanceAvailableNew > 0 && (sliderIndex !== (existingAllSliderArray.length - 1))){
            ...
            console.log(`then adjusted to sliderIndex ${lastSlider} with existingValue + balanceAvailable = ${newAllSliderArray[lastSlider]} + ${balanceAvailableNew} = ${newAllSliderArray[lastSlider] + balanceAvailableNew} or min ${sliders[lastSlider].min}  `)
        ...
        if(latestTotalOfAllSliders !== totalValue){
          ...
          if(latestTotalOfAllSliders > totalValue){
            console.log(`Total of all sliders ${newAllSliderArray} = ${latestTotalOfAllSliders} crossed the limit ${totalValue} hence decrease from i-1`); 
            ...
            for(let i=sliderIndex-1;i>=0;i--){
              if((newAllSliderArray[i]-sliders[i].min) - valueToBeDecreased >= 0) {
                console.log(`Above slider can accomodate the entire over limit value itself ${newAllSliderArray[i]} - ${valueToBeDecreased} = ${newAllSliderArray[i] - valueToBeDecreased} or min ${sliders[i].min}`)
                ...
                console.log(`Adjusted until min value ${sliders[i].min} and continue adjusting balance ${valueToBeDecreased} i-1`)
              }
            }
          } else {
            console.log(`Total of all sliders ${newAllSliderArray} = ${latestTotalOfAllSliders} not reached the limit ${totalValue} hence increase from i-1 only sliderIndex is last`); 
            ...
            if(sliderIndex === (existingAllSliderArray.length -1)) {
              for(let i=sliderIndex-1;i>=0;i--){
                console.log(`${newAllSliderArray[i]} + ${valueToBeIncreased} = ${newAllSliderArray[i] + valueToBeIncreased}`)
      ...
      for(let i=sliderIndex+1;i<existingAllSliderArray.length;i++){
        if(total) {
          ...
          if(balanceAvailable >= difference){
            ...
            console.log(`balanceAvailable ${balanceAvailable} >= difference ${difference} hence proposional with differnce \n${newAllSliderArray[i]} - (${existingAllSliderArray[i]}/${total}) * ${difference} = ${newValue} or ${minSliderValue}`);
          }else{
            console.log(`balanceAvailable ${balanceAvailable} < total ${total} hence proposional with balanceAvailable`);
            let newValue = (isLoop ? newAllSliderArray[i] : 0 ) + Number(Math.floor((existingAllSliderArray[i]/total) * balanceAvailable));
            newAllSliderArray[i] = newValue > minSliderValue ? newValue : minSliderValue;
            console.log(`(${existingAllSliderArray[i]}/${total}) * ${balanceAvailable} = ${newValue} or ${minSliderValue}`);
          }
        }
      }
